...
const UserProfile = () => {
  ...
  const add = async () => {
    ...
    if (resume === null || Location.length < 3 || PrimaryRole.length < 3 || Statement.length < 3) {
      return;
    }
    //get cv file url
    setisLoading(true)
                  ...
                  <div className='flex'>
                    {resume === null && (
                      <>
                        ...
                        <input type='file' id='file' accept="application/pdf" required ref={inputFile} onChange={(e) => setResume(e.target.files[0])} style={{ display: 'none' }} />
                      </>
                    )}
                    {resume !== null && (
                      <>
