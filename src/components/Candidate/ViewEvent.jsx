...
const ViewEvent = () => {
    ...
    const saveEvent = async () => {
        if (savedEvent.includes(eventApply.viewData._id)) {//delete job
            try {
    ...
    return (
        <>
            <div className={`${authModal && 'h-screen overflow-hidden'}`}>
                <Navbar />
                <header className='relative h-72 bg-[#14163A] flex  items-center'><br />
                    <div className='pt-10 pl-10 pr-10 w-full '>
                        <div className='box-content h-48 w-full '>
                            <div className='relative pl-4 pr-4 pt-4 h-full '>
                                ...
                                <div className='pl-48'>
                                    <div className='w-3/4'>
                                        <h1 className='text-white font-sans text-4xl font-bold'>{eventApply.viewData.eventTitle}</h1><br />
                                        <h1 className='text-white font-sans text-2xl font-bold'>Company: {eventApply.viewData.company.name}</h1>
                                        <h1 className='text-white font-sans text-xl font-bold'>Category: {eventApply.viewData.company.name}</h1>
                                    </div>
                                </div>
                                <h1 className='text-white absolute right-4 top-6 '>{eventApply.viewData.createdAt}</h1>
                                {/* Saved Button */}
                                ...
                                {/* bloaked button */}
                                {user === null && (
                                    <button title="login to save" disabled={true} className='cursor-not-allowed'>
                                        <h1 className='text-white absolute float-right right-10 bottom-0'>
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                ...
                {isLoading ? <Loading /> : (<>
                    <div className='flex'>
                        <div className='pl-36 w-1/2 pr-6'>
                            <div className='box-content h-auto w-full  pt-6 '>
                                ...
                                <p className=''>
                                    {eventApply.viewData.description}
                                </p>
                            </div>
                        </div>
                    ...
                    <div className='pl-36 pr-10 pb-6'>
                        <div className='box-content h-14 w-full pt-2'>
                            <div class="flex ">
                                <div className='flex text-base text-slate-600 font-sans font-medium'><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                    ...
                                    {eventApply.viewData.location}</div>
                                <div className='flex text-base pl-20 text-slate-600 font-sans font-medium'><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z" />
                                </svg>
                                    {moment(eventApply.viewData.date).utc().format('YYYY-MM-DD')} | {moment(eventApply.viewData.date).utc().format('h:mm a')}
                                </div>
                            </div>
                            <div className='flex text-base pt-4 text-slate-600 font-sans font-medium'><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M3.75 21h16.5M4.5 3h15M5.25 3v18m13.5-18v18M9 6.75h1.5m-1.5 3h1.5m-1.5 3h1.5m3-6H15m-1.5 3H15m-1.5 3H15M9 21v-3.375c0-.621.504-1.125 1.125-1.125h3.75c.621 0 1.125.504 1.125 1.125V21" />
                            </svg>
                                {eventApply.viewData.deliveryType}</div>
                        </div>
                    </div>
                ...
                <div className='flex  justify-center pb-4'>
                    <h1 className='text-black font-sans text-4xl font-bold'>Take Part in the Event</h1>
                </div>
                                            ...
                                            <div className='col-span-6'>
                                                ...
                                                <input
                                                    ...
                                                    {...register('role', {
                                                        ...
                                                        minLength: {
                                                            value: 3,
                                                            message: 'Role/Ocupation must be at least 3 characters',
                                                        },
                                                    })}
                                                />
                                                {errors.role && (
                                                ...
                                                <div className='pb-10 pt-10 flex justify-center'>
                                                    <button type="submit" class="focus:outline-none text-white font-medium rounded-lg text-sm px-10 py-2.5 mr-2 mb-2 bg-red-600 hover:bg-red-700 focus:ring-red-900">
                                                        Sign-Up
                                                    </button>
                                                </div>
                                            </div>
