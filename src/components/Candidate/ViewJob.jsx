...
const ViewJob = ({ setViewjob }) => {
  ...
  const applyJob = async () => {
    ...
    const JobID = jobApply.viewData.job._id
    const CompanyID = jobApply.viewData.job.company._id
  ...
  //save method
  const saveJob = async () => {
    if (savedJobs.includes(jobApply.viewData.job._id)) {//delete job
      try {
        const API_URL = `candidate/delsaveJob/${jobApply.viewData.job._id}`
      ...
      try {
        ...
        const response = await api.post(API_URL, {
          userID: user.userId,
          JobID: jobApply.viewData.job._id
        })
        toast.info(response.data.msg, { theme: 'dark' })
      } catch (error) {
        toast.error("error", { theme: 'dark' })
          ...
          <div className='pt-10 pl-10 pr-10 w-full '>
            <div className='box-content h-48 w-full '>
              <div className='relative pl-4 pr-4 pt-4 h-full '>
                <img
                  src={jobApply.viewData.job.company.photoUrl}
                  alt='job-img'
                  className='float-left object-cover bg-white h-36 w-36 rounded-lg shadow-2xl'
                />
                <div className='pl-48'>
                  <div className='w-3/4'>
                    <h1 className='text-white font-sans text-4xl font-bold'>{jobApply.viewData.job.jobTitle}</h1><br />
                    <h1 className='text-white font-sans text-2xl font-bold'>Company: {jobApply.viewData.job.company.name}</h1>
                    <h1 className='text-white font-sans text-xl font-bold'>Category: {jobApply.viewData.job.jobCategory}</h1>
                  </div>
                </div>
                <h1 className='text-white absolute right-4 top-6 '>{jobApply.viewData.job.createdAt}</h1>
                <button onClick={applyJob} className='bg-red-700 hover:bg-red-500  text-white absolute float-right right-4 bottom-0 px-5 py-2.2 rounded-sm '>Apply</button>
                {/* Saved Button */}
                {savedJobs.includes(jobApply.viewData.job._id) && (
            ...
            <div className='box-content h-auto w-full pt-2 '>
              <div className='pt-6 '>
                ...
                <ul class="list-disc pl-4">
                  {jobApply.viewData.job.jobRequirements.map((data) => (
                    <li>{data}</li>
                  ))}
                </ul>
              </div>
              <div className='pt-6 '>
                ...
                <ul class="list-disc pl-4">
                  {jobApply.viewData.job.jobExpectations.map((data) => (
                    <li>{data}</li>
                  ))}
                </ul>
              </div>
