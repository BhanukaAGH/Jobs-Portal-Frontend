  ...
  const getAllJobs = async () => {
    ...
    setJobs(
      response.data.jobs.filter(
        (data) =>
          data.jobType.includes(experienceF) &&
          data.jobCategory.includes(categoryF) &&
          data.workType.includes(medialitiyF) &&
          data.company.name.includes(companyF)
      )
    )
  ...
  return (
    <>
      <p className='text-sm italic'>
        {jobs.length === 0 && <>No Matching Results found</>}
      </p>
